...
// ─────────────────────────────────────────────────────────────────────
// Search — `.search{border:1px solid var(--line-2);height:46px}`
// ─────────────────────────────────────────────────────────────────────

/// The mockup's search box: a hairline rectangle, a glyph, and grey prompt
/// text. No fill, no radius beyond the design's 2px, no elevation.
///
/// It takes a live [TextEditingController] rather than owning its own text so a
/// screen can keep the debounce, the clear button and the query state it
/// already has — this is presentation, not a new search.
class GaiaSearchField extends StatelessWidget {
  const GaiaSearchField({
    super.key,
  ...
  Widget build(BuildContext context) {
    return Container(
      height: GaiaSpacing.fieldHeight,
      padding: const EdgeInsets.symmetric(horizontal: 14),
      decoration: BoxDecoration(
        border: Border.all(color: GaiaColors.line2),
        borderRadius: GaiaRadius.all,
      ),
      child: Row(
        children: [
          const Icon(Icons.search, size: 17, color: GaiaColors.text3AA),
          const SizedBox(width: 10),
          Expanded(
            child: TextField(
              controller: controller,
              onChanged: onChanged,
              style: GaiaType.sans(size: 14, color: GaiaColors.text),
              cursorColor: GaiaColors.champagne,
              cursorWidth: 1,
              decoration: InputDecoration(
                ...
                hintText: hint,
                hintStyle: GaiaType.sans(size: 14, color: GaiaColors.text3AA),
              ),
            ),
          ),
          if (onClear != null)
                  ...
                  : GestureDetector(
                      ...
                      child: const Padding(
                        ...
                        child: Icon(Icons.close,
                            size: 16, color: GaiaColors.text2),
                      ),
                    ),
            ),
        ],
